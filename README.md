...
**Three. Independent-build divergence as a spec-hole detector.** Have several models implement the same spec independently and diff their outputs across many inputs. Every disagreement is a question the spec failed to answer. The divergence count is a direct measure of spec ambiguity, and it sees holes no single reviewer thinks to look for, because a second reviewer shares the first one's blind spots, while builds that do not share training do not.
...
The argument is no longer whether AI reviews are worth it. It is what fraction of the real defect total a process kills, measured against a ground truth you injected, and the taxonomy you need to inject is finite, because the software is composed from a basis you can write down.
